...
FEATURES: tuple[FeatureSpec, ...] = (
    ...
    _spec(
        ...
        "حسب الخدمات المسحوبة وينشر أرخص 10 خدمات في كل نوع بسعر تكلفة المزود "
        "+ هامش الربح، ويتجاهل الخدمات بلا سعر (سيرفر/عناوين)، دون تكرار أو "
        "مساس بالمنتجات اليدوية.",
        True,
        max_per_section=10,
        margin_percent=50,
    ...
    _spec(
        "subscriptions_auto_sync",
        "مزامنة الاشتراكات الرقمية (ggsoma)",
        "Digital Subscriptions Auto Sync",
        "الاشتراكات",
        "يسحب كتالوج مزود ggsoma المرتبط (LINK/كوبون/حساب جاهز) وينشر "
        "منتجاته تلقائياً في قسم الاشتراكات الرقمية — قسماً فرعياً لكل "
        "تطبيق — بسعر تكلفتك + هامش الربح، مع تحديث التوفر والأسعار دون "
        "تكرار أو مساس بالمنتجات اليدوية. يعمل عند كل إقلاع ومن زر اللوحة.",
        True,
        margin_percent=23,
        auto_on_startup=True,
    ),
    ...
    _spec(
        "top_buyers",
        "🏆 ألقاب المشترين النشطين (لوحة تفاعلية)",
        "Top Buyers Leaderboard",
        "التفاعل",
        "يعرض أفضل 10 مشترين هذا الأسبوع بألقاب ذهبية/فضية/برونزية "
        "مترقية للجميع — يحفز التنافس ويزيد حجم المشتريات.",
        False,
        max_players=10,
        show_week=True,
    ),
    _spec(
        ...
        "Daily Spin Wheel",
        "التفاعل",
        "لفّة مجانية يومياً: رصيد، نقاط ولاء، أو «حظ أوفر». الجوائز "
        "واحتمالاتها يتحكم بها الأدمن بالكامل. ترفع فتح البوت يومياً.",
        False,
        spins_per_day=1,
        free_always=True,
    ),
    _spec(
        "price_alerts",
        "🔔 إنذارات انخفاض السعر",
        "Price Drop Alerts",
        "الأرقام",
        "المستخدم يضبط تنبيه: «نبهني حين ينزل سعر واتساب تركيا تحت X$» "
        "ويُشعَر فوراً برابط شراء مباشر — يشتري باللحظة المناسبة.",
        False,
    ...
    _spec(
        "weekly_challenges",
        "🏅 التحديات الأسبوعية بجوائز",
        "Weekly Challenges",
        "التفاعل",
        "تحدٍّ أسبوعي يحدده الأدمن (عدد الطلبات/الصرف/الحضور) بمكافأة "
        "رصيد أو نقاط. يرفع عدد الطلبات كل أسبوع.",
        False,
        weekly_limit=2,
        auto_claim=True,
    ),
    _spec(
        "number_resale_market",
        "🔄 سوق الأرقام المستعملة",
        "Used Number Resale",
        "السوق",
        "المستخدم يبيع رقمه الاشترائي (قبل انتهاء وقته) بمقابل رصيد "
        "لمستخدم آخر — يقلل المرتجعات ويجلب عمولة لكل عملية بيع.",
        False,
    ...
    _spec(
        "provider_reviews",
        "⭐ تقييم المزودين (بعد الطلب)",
        "Provider Reviews",
        "الثقة",
        "تقييم 1-5 نجوم للمزود بعد كل طلب مكتمل. يظهر متوسط التقييم "
        "للمستخدم قبل الشراء — ثقة أكبر وشكاوى أقل.",
        False,
        require_comment=False,
        min_rating_provider=3,
        show_avg=True,
    ),
    _spec(
        "monthly_report",
        ...
        "النمو",
        "في أول كل شهر يُرسل البوت لكل مستخدم تقريراً شخصياً: "
        "عدد الطلبات، الإنفاق، أكثر الدول/الخدمات، نقاط الولاء. "
        "يعيد إحياء حوار المستخدمين الخاملين.",
        False,
        send_day_of_month=1,
        send_time_utc="09:00",
        include_top_items=3,
    ...
    _spec(
        ...
        "Offer Expiry Alerts",
        "الإشعارات",
        "قبل ساعة من انتهاء أي عرض خاص (24 ساعة)، يرسل البوت تذكيراً "
        "لكل من شاهد العرض — مبيعات في آخر ساعة.",
        False,
        remind_minutes_before=60,
        only_viewers=True,
    ),
    _spec(
        "code_screenshot_delivery",
        "📸 تسليم الكود كصورة",
        "Code Screenshot Delivery",
        "الأرقام",
        "بدل النص وحده، يُرسل الكود النهائي كصورة منسقة أنيقة فيها "
        "الرقم/الكود/الخدمة — أسهل للحفظ والمشاركة بين الفريق.",
        False,
    ),
    _spec(
        ...
        "الاقتصاد",
        "بطاقة حساب تفاعلية تعرض: أكثر خدمة اشتريتها، الإنفاق هذا الشهر، "
        "عدد الطلبات النشطة، آخر شراء، أكثر دولة — تقرير بصري كامل.",
        True,
        show_stats=True,
    ),
    ...
    _spec(
        "mobile_topup_gift",
        "🎁 اشحن لأهلك (تحويلات الشتات)",
        "Mobile Topup Gift",
        "الاقتصاد",
        "يسجّل طلب شحن لسلفك في الوطن: اختيار المشغل والمبلغ ورقم المستلم. "
        "طلب مراجعة يظهر للوحة الأدمن فوراً — مخصص للشتات.",
        False,
        min_amount_usd=1,
        max_amount_usd=100,
    ...
    _spec(
        "campaign_codes",
        "🎟 أكواد الحملات مع تتبّع المصدر",
        "Campaign Codes",
        "النمو",
        "كواد خصم مخصصة لكل حملة إعلانية بوسم تتبّع (حركة#حملة). يُنشئها "
        "الأدمن من اللوحة، ويُعرف منها كم طلباً جلبت كل إعلان.",
        False,
        max_uses_per_code=500,
    ),
    _spec(
        "auto_failover",
        "🛡 العزل الذاتي للمزود المعطوب",
        "Auto Provider Failover",
        "الرشق",
        "يراقب فشل المسارات المتتالية لكل منتج، وإذا تكرر الفشل عزل المسار "
        "المعطوب تلقائياً وتفعيل الاحتياط دون انتظار تدخل الأدمن.",
        False,
        failure_threshold=3,
    ...
    _spec(
        "flash_sale_timer",
        "⏱ مؤقّت لحظي على العروض الخاصة",
        "Flash Sale Timer",
        "السوق",
        "عدّ تنازلي لحظي (ساعة:دقيقة:ثانية) على كل عرض خاص نشط في قائمة "
        "العروض — يرفع ضغط الشراء قبل نهاية العرض.",
        True,
    ),
    _spec(
        "vip_tiers",
        "👑 مستويات VIP ومضاعف الكاشباك",
        "VIP Tiers & Cashback",
        "الاقتصاد",
        "مستويات تُحتسب تلقائياً من إجمالي إنفاق المستخدم، وكل مستوى يكسب "
        "الواحد مضاعف كاشباك أعلى — مكافأة الولاء ورفع أرقام المشتريات.",
        False,
        tiers_json='[{"min_spent_usd":0,"tier":0,"name":"عادي","cashback_mult":1.0},'
                   '{"min_spent_usd":25,"tier":1,"name":"برونزي","cashback_mult":1.1},'
                   '{"min_spent_usd":100,"tier":2,"name":"فضي","cashback_mult":1.25},'
                   '{"min_spent_usd":300,"tier":3,"name":"ذهبي","cashback_mult":1.5}]',
