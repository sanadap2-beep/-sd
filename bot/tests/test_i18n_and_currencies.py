...
def test_every_key_used_in_code_exists_in_locales():
    ...
    for path in list((_ROOT / "handlers").rglob("*.py")) + list(
        ...
        for match in re.finditer(
            r'(?:I18nService\.t|\bt)\(\s*["\']([a-z][a-z_0-9]*)["\']',
            source,
            re.S,
        ):
...
def test_currency_keyboard_returns_telegram_markup():
    ...
    assert any(
        button.callback_data == "currency:set:EUR"
...
async def test_convert_and_format_display_currency():
    async with async_session_maker() as session:
        ...
        usd = await CurrencyService.format_from_usd(
            Decimal("5"), "USD", session
        )
        assert usd == "$5.00"
...
async def test_dual_format_for_user():
    async with async_session_maker() as session:
        ...
        dual = await CurrencyService.format_dual(
            Decimal("2"),
            user,
            session,
        )
