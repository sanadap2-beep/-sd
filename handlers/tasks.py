...
async def tasks_home(callback: CallbackQuery, session, db_user):
    ...
    for task in tasks:
        ok, reason = await TaskService.eligibility(
            session,
            db_user.id,
            task,
            callback.bot,
...
async def task_do(
    ...
    if task.task_type in (
        TaskType.REPORT_PROVIDER,
        TaskType.TRANSLATE_TEXT,
    ) or task.verification == TaskVerification.ADMIN_APPROVAL:
        await callback.answer()
    ...
    # الإصلاح:
    # كان يتم استدعاء tasks_home(callback) فقط،
    # مع أن الدالة تحتاج session و db_user أيضاً.
    await tasks_home(
        callback,
        session,
        db_user,
...
async def task_content(
    ...
    if task is None or not text:
        ...
        return await message.answer(
            "⚠️ انتهت الجلسة، أعد المحاولة."
        )

    try:
        submission = await TaskService.submit(
            session,
            db_user.id,
            task,
            text,
...
async def task_captcha(
    ...
    if answer != int(data.get("captcha_answer", -1)):
        ...
        return await message.answer(
            "❌ إجابة خاطئة. أعد المحاولة من قائمة المهام."
        )
